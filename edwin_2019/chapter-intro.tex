...
通过设计和实现仿冒应用筛选框架\mytool，笔者顺利从Janus搜寻到了超过15万条数据条目作为原始数据，其中每条数据条目代表Janus从应用市场上获得的一个应用样本。
...
本文的创新点如下：
\begin{itemize}
	\item 设计并实现了应用过滤框架\mytool，并对收集到的仿冒应用数据进行挖掘和案例分析，提供了关于仿冒应用填补了移动黑灰产研究中关于仿冒应用的研究空白；
...
如\autoref{fig:Workflow}所示，本课题通过三个主要部分完成实证研究：
...
借助本文的自动化仿冒应用过滤框架\mytool，作者获得了大量仿冒应用的数据。

2)\ \emph{仿冒应用特征解读} \quad
本文对搜集到的仿冒样本数据进行分析，从三个视角完成了一次大规模数据挖掘。
这些视角分别是仿冒的基本应用特征、影响仿冒应用数量的因素和仿冒应用的发展轨迹，由浅入深，帮助读者理解仿冒应用的生态。
之后，笔者从收集的数据中选择了3个案例进行更深入的分析。
这三个案例在支持数据挖掘中的发现之外，揭示了更多仿冒应用开发者的行为特征。

3)\ \emph{仿冒应用评论分析} \quad
在这个部分，本文从第三方应用市场中随机选取一部分应用，爬取了用户对它们的所有历史评价，然后筛选出其中对仿冒应用的评价并分析，以期了解用户对这些应用持有的态度。
此基础上，本文还检测了仿冒应用与排名欺诈行为的关系，揭示移动黑灰产不同环节之间的关联。


\section{本文组织结构}
本文共分为五章，环绕着本研究的数据搜集、分析过程和分析结果展开，各章节内容如下：

\fullref{chp:intro} 主要提供了本文的研究背景、相关工作、研究意义、主要工作和研究概览。
...
\fullref{chp:fakerevealer} 解释了仿冒应用的数据收集方式，阐述了仿冒应用过滤框架\mytool 的运行流程，并详细介绍了其中三个组件（\componentA、\componentB 和\componentC）的设计与实现，最后提供仿冒应用数据概览。

\fullref{chp:discoveries} 从多个视角分类提出并解说针对这批仿冒应用数据得到的发现。
这些视角包括仿冒应用特征、影响仿冒应用数量的因素和仿冒应用的发展轨迹，每个视角都被进一步分解成了多个不同的研究问题。
最后，笔者筛选出了数据中具有代表性的一些案例进行案例分析，以案例进一步佐证分析结果的正确性。
