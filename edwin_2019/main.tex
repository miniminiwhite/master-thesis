...
% 基本信息\documentclass[options]{class}
\newcommand{\TheisName}[0]{基于Android仿冒应用的实证研究}
\newcommand{\TheisNameEn}[0]{An Empirical Study on Android Fake Apps}

\def\yearOfGrduation{2020} % 毕业年份
\def\monthOfGraduationNum{10} % 毕业月份
\def\monthOfGraduationEng{October} % 毕业月份 (英文)
...
% \clearPaperPage
\input{chapter-research-settings}
\clearPaperPage

\input{chapter-discoveries-basic}
\clearPaperPage

\input{chapter-discoveries-behavior}
\clearPaperPage

\input{chapter-framework-prototype}
\clearPaperPage
...
% 致谢。匿名与非匿名用不同版本
\ifx\anonymous \undefined
    \input{D3-ACKNOWLEDGEMENT}
\else
    \input{D3-ACKNOWLEDGEMENT-ANONYMOUS}
\fi
\clearPaperPage
