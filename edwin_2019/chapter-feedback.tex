...
在我们将$T_1$设置为0.6的情况下，{\textcolor{orange}{[Edwin: 实验数据还在服务器上运行，等运行出来之后再补上结果]}
...
值得一提的是，在我们收集到的评论数据中，评论重合率最高的是一个包名为\emph{com.android.apin}的应用，其应用重合率达到了91.41\%。
...
\begin{figure}[htbp]
	\centering
	\subfloat[\emph{com.jhwl.fjxa}部分评论\label{fig:cmt-sample-1}]{\includegraphics[width=0.8\textwidth]{./Figures/edwin-cmt-sample-1.jpg}}\hfill

	\subfloat[\emph{com.arsenal.FunWeather}部分评论\label{fig:cmt-sample-2}]{\includegraphics[width=0.8\textwidth]{./Figures/edwin-cmt-sample-2.jpg}}\hfill
    \caption{应用评论取样（一）}
    \label{fig:cmt-samples-1}
\end{figure}

\autoref{fig:cmt-sample-1}展示了我们对\emph{com.arsenal.FunWeather}和\emph{com.jhwl.fjxa}两款应用好评取样的结果。
可以明显看出，两款App都有明显的评论重复现象。
同时，我们也对\emph{com.sscwap}的应用进行了取样，样本可见\autoref{fig:cmt-samples-2}。
虽然图中的评论看上去也比较接近，但这其实是由于我们按照评论内容对这些评论进行了一次排序。
如果我们结合日期数据观察，就能发现这些数据其实是由用户在比较分散的时间发出的，所以说这些看上去稍微类似的评论，并不一定存在关联关系，因此我们不倾向于认为这款App购买了排名欺诈服务。
反观\autoref{fig:cmt-sample-1}中的评论，首先，有多条长评论高度相似、甚至一模一样，这在真实案例中是不太可能会存在的情况；其次，\autoref{fig:cmt-sample-2}中多条近似、相同的评论是在十分接近的时间里被发表的，进一步加深了他们之间存在关联的可能性，所以我们十分有理由相信这两款应用的确存在排名欺诈的行为。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{./Figures/edwin-cmt-sample-3.jpg}
    \caption{应用评论取样（二）（\emph{com.sscwap}）}
    \label{fig:cmt-samples-2}
\end{figure}

从结果看，基于评论内容相似度的排名欺诈排查方法还是有一定效果的；而基于用户可信度权重的排查方法效果欠佳。
值得注意的是，我们还随机抽取了一些发布相同好评的不同用户进行追查，结果发现不少用户在我们收集到的数据集中的评论数只有一到两条，这很有可能是提供排名欺诈服务的商家规避检测的一种策略，同时也是导致基于用户可信度权重排查方法效果不太理想的原因。
